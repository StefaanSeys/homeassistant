...
class StylingLayoutCard extends Polymer.Element {
  ...
  _build() {
    ...
    let maxlen = 0;
    // added by Stefaan to make styling possible
    let z = 0;
    cols.forEach( c => {
      const cEl = document.createElement('div');
      cEl.classList.add('column');
      // added by Stefaan to make styling possible
      if (this.config.styles && Array.isArray(this.config.styles) && z <= this.config.styles.length)
        Object.assign(cEl.style, this.config.styles[z]);
      // end Stefaan block
...
customElements.define('styling-layout-card', StylingLayoutCard);
});

window.setTimeout(() => {
  if(customElements.get('card-tools')) return;
  customElements.define('styling-layout-card', class extends HTMLElement{
